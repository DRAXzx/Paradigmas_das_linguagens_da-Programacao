def nome_para_numero(palavra):
    # Dicionário para mapear nomes de números em inglês para seus valores numéricos
    numeros_em_ingles = {
        'zero': '0', 'one': '1', 'two': '2', 'three': '3', 'four': '4',
        'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'nine': '9'
    }
    
    # Verifica se a palavra é uma combinação de dois números
    for nome, digito in numeros_em_ingles.items():
        if palavra.startswith(nome):  # Verifica se a palavra começa com um número
            resto = palavra[len(nome):]  # Verifica o restante da palavra
            if resto in numeros_em_ingles:
                # Se o restante também é um número válido, combine os dois
                return digito + numeros_em_ingles[resto]
    
    # Verifica se é um número simples
    if palavra in numeros_em_ingles:
        return numeros_em_ingles[palavra]
    
    return None  # Se não for um número válido em inglês

def soma_calibracoes(lista_palavras):
    soma_total = 0
    ...
    for palavra in lista_palavras:
        ...
        # Converte nomes de números para seus equivalentes numéricos
        numero_convertido = nome_para_numero(palavra)
        if numero_convertido:
            # Se for um número válido ou combinado, usa diretamente o valor
            primeiro_digito = numero_convertido[0]
            ultimo_digito = numero_convertido[-1]
        
        # Verifica os dígitos numéricos se não foi convertido pelo nome
        for char in palavra:
            if char.isdigit():
        ...
        if primeiro_digito is not None and ultimo_digito is not None:
            numero_calibracao = int(primeiro_digito + ultimo_digito)
            soma_total += numero_calibracao
    
    return soma_total

# Exemplo de uso
lista_palavras = ['ckmb52fldxkseven3fkjgcbzmnr7',
...
resultado = soma_calibracoes(lista_palavras)
print("Soma dos valores de calibração extraídos:", resultado)